...
function getAuth(auth){
    ...
    for (var i = 0; i < recipents.length; i++) {
        ...
        // const myHTML = `Dear ${recipents[i].name}<br><h1>Merry Christmas.</h1>`;
        const myHTML = htmlTemplate;
        
        var obj = new Mail(auth, recipents[i].email, 'New Subject', myHTML); // , 'mail');
        //'mail' is the task, if not passed it will save the message as draft.
        //attachmentSrc array is optional.
        obj.makeBody(); // This will send/make-draft the mail to the recipent.
    }
